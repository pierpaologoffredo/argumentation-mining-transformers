...
# OUTPUT_DIR=./output/deberta-v3_e$e-b=$b-sl$MAX_SEQ_LENGTH-lr$LEARNING_RATE
    ...
    for b in "${batch[@]}"; do
      for m in "${max[@]}"; do
        for l in "${lr[@]}"; do
          ...
          python ./run_acta.py \
            --input-dir $INPUT_DIR \
            --output-dir "./output/$MODEL-epoch=$e-batch=$b-$TASK_TYPE=$m-lr=$l-loop_i=$i" \
            --task-type $TASK_TYPE \
            --model $MODEL \
            --cache-dir $CACHE_DIR \
